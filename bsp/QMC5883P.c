...
static float QMC5883P_UnwrapAngleDeg(float current_deg)
{
    ...
    static float continuous_deg = 0.0f;
    float delta_deg;
...
HAL_StatusTypeDef QMC5883P_Init(void)
{
    ...
    if (QMC5883P_Read(QMC5883P_REG_CHIP_ID, &id) != HAL_OK)
        return HAL_ERROR;
    ...
    if (QMC5883P_Write(QMC5883P_REG_CONTROL_2, QMC5883P_SOFT_RST) != HAL_OK)
        return HAL_ERROR;
     ...
     */
    if (QMC5883P_Write(QMC5883P_REG_SIGN, QMC5883P_SIGN_REG_VALUE) != HAL_OK)
        return HAL_ERROR;
#endif

    /* 先配 Control_2：Set/Reset On + 8G */
    if (QMC5883P_Write(QMC5883P_REG_CONTROL_2, QMC5883P_CTRL2_INIT) != HAL_OK)
        return HAL_ERROR;

    /* 再配 Control_1：Normal mode + 200Hz + 最大 OSR */
    if (QMC5883P_Write(QMC5883P_REG_CONTROL_1, QMC5883P_CTRL1_INIT) != HAL_OK)
        return HAL_ERROR;

    /* 回读校验 Control_2 */
    if (QMC5883P_Read(QMC5883P_REG_CONTROL_2, &verify) != HAL_OK)
        return HAL_ERROR;

    if (verify != QMC5883P_CTRL2_INIT)
        return HAL_ERROR;

    /* 回读校验 Control_1 */
    if (QMC5883P_Read(QMC5883P_REG_CONTROL_1, &verify) != HAL_OK)
        return HAL_ERROR;

    if (verify != QMC5883P_CTRL1_INIT)
        return HAL_ERROR;
...
HAL_StatusTypeDef QMC5883P_GetData(int16_t *x, int16_t *y, int16_t *z)
{
     ...
     * 所以这里先判断状态，再立即读数据。
     */
    if (QMC5883P_Read(QMC5883P_REG_STATUS, &status) != HAL_OK)
        return HAL_ERROR;
     ...
     */
    if (QMC5883P_ReadBytes(QMC5883P_REG_XOUT_LSB, buf, 6) != HAL_OK)
        return HAL_ERROR;
